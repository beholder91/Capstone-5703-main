...
def train(model, train_loader, val_loader, loss_function, optimizer, folder_name, plot_name, num_epochs=10, patience=10):
    ...
    save_history(f'output/mvsa/Adam/all_histories.json', f'{plot_name}', history)
...
def trainmodel(mvsa_label_path, mvsa_image_path, configs):
    ...
    folder_name = f'output/mvsa/Adam/{modelname}/drop-{dropout_rate} lr-{learning_rate} gamma-{gamma}'
    if not os.path.exists(folder_name):
        os.makedirs(folder_name)

    plot_name = f"mvsa_multi {modelname} dr-{dropout_rate} lr-{learning_rate}"

    # log_filename = f"/content/{dropout_rate}_{learning_rate}_{gamma}.log"
    log_filename = f"output/mvsa/Adam/{modelname}/{dropout_rate}_{learning_rate}_{gamma}.log"
    setup_logger(log_filename)
    logging.info(f"\n{modelname}\ndropout: {dropout_rate}\nlearning rate: {learning_rate}\ngamma: {gamma}")

    print("\t ++++ Preprocessing dataset ... ")
    df = process_multi_label(mvsa_label_path, mvsa_image_path)
    # print(df.head())
...
def run_mvsa_adam():
    mvsa_multiple_data_path = 'CS15_2_virtual/DATASET/MVSA/data'
    mvsa_multiple_label_path = 'CS15_2_virtual/DATASET/MVSA/labelResultAll.txt'
    # mvsa_multiple_data_path = 'CS15_2_virtual/DATASET/MVSA_multi/MVSA/data'
    # mvsa_multiple_label_path = 'CS15_2_virtual/DATASET/MVSA_multi/MVSA/labelResultAll.txt'
    # mvsa_multiple_data_path = '/content/MVSA/data'
    # mvsa_multiple_label_path = '/content/MVSA/labelResultAll.txt'
    ...
    for config_name, config in mvsa_configs.items():
        print(f"\n============ Train MVSA {config_name} ============ ")
        trainmodel(mvsa_multiple_label_path, mvsa_multiple_data_path, config)

    # Load histories from the JSON file
    all_histories = load_histories('output/mvsa/Adam/all_histories.json')
    ...
    for keyword in keywords:
        ...
        plt.ylabel(keyword)
        plt.legend()
        plt.savefig(os.path.join('output/mvsa/Adam', f"{keyword}.png"))
        # plt.show()
