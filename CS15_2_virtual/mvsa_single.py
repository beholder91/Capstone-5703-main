...
def train(model, train_loader, val_loader, loss_function, optimizer, folder_name, plot_name, num_epochs=10, patience=10):
    ...
    save_history(f'output/mvsasingle/SGD/all_histories.json', f'{plot_name}', history)
...
def trainmodel(mvsa_label_path, mvsa_image_path, configs, modelname):
    ...
    folder_name = f'output/mvsasingle/SGD/{modelname}/drop-{dropout_rate} lr-{learning_rate} gamma-{gamma}'
    ...
    log_filename = f"output/mvsasingle/SGD/{modelname}/{dropout_rate}_{learning_rate}_{gamma}.log"
...
def run_mvsa_single():
    ...
    all_histories = load_histories('output/mvsasingle/SGD/all_histories.json')
    ...
    for keyword in keywords:
        ...
        plt.ylabel(keyword)
        plt.legend()
        plt.savefig(os.path.join('output/mvsasingle/SGD', f"{keyword}.png"))
        # plt.show()
